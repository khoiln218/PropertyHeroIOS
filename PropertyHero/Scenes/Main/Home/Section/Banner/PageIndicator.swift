    ...
    public static func withSlideshowColors() -> UIPageControl {
        ...
        if #available(iOS 13.0, *) {
            pageControl.currentPageIndicatorTintColor = UIColor { traits in
                (traits.userInterfaceStyle == .dark ? .white : UIColor(hex: "#f05323")) ?? .lightGray
            }
        } else {
            pageControl.currentPageIndicatorTintColor = UIColor(hex: "#f05323") ?? .lightGray
        }
        
        if #available(iOS 13.0, *) {
            pageControl.pageIndicatorTintColor = UIColor { traits in
                traits.userInterfaceStyle == .dark ? .systemGray : .white
            }
        } else {
